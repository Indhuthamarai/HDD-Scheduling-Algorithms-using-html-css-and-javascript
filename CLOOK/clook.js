...
function clook_man(requestSequenceClook, headClook, direction) {
  ...
  let requestSequenceClookSorted = tmpAry.sort(function (a, b) {
      return a - b;
  });

  //loop will break when headClook is less than ith element and tmp=i
  for (let i = 0; i < requestSequenceClookSorted.length; ++i) {
  ...
  if (direction === "Right") {
      for (let i = tmp; i < requestSequenceClookSorted.length; ++i) {
      ...
      totalSeekCountClook =
          Math.abs(requestSequenceClookSorted[requestSequenceClookSorted.length - 1] - headClook +
      ...
      for (let i = requestSequenceClookSorted.length - 1; i >= tmp; --i) {
          requestFinalOrderClook.push(requestSequenceClookSorted[i]);
      }
