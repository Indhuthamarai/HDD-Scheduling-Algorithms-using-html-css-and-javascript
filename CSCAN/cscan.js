...
function cscan_man(requestSequenceCscan, headCscan, direction) {
  ...
  let requestSequenceCscanSorted = tmpAry.sort(function (a, b) {
    return b - a;
  });
  ...
  if (direction === "Right") {
    ...
    for (let i = requestSequenceCscanSorted.length - 1; i >= tmp; --i) {
  ...
  // for direction == left
    ...
    for (let i = tmp; i <= requestSequenceCscanSorted.length - 1; ++i) {
      requestFinalOrderCscan.push(requestSequenceCscanSorted[i]);
    }
